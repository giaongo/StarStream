...
async def edit_streaming_url():
    ...
    if form.validate():
        QuartSIO.get_instance().streaming_url = form.streaming_url.data
        edit_result = await update_streaming_url(form.streaming_url.data)
        if not edit_result:
            return {'msg': 'error updating streaming url'}, 400
        return {'msg': f'streaming url updated successfully with url {QuartSIO.get_instance().streaming_url}'}, 200
...
async def edit_viewing_url():
    ...
    if form.validate():
        ...
        return {'msg': f'viewing url updated successfully with url {form.viewing_url.data}'}, 200
